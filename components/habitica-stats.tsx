...
function drawStatsImage(ctx: CanvasRenderingContext2D, stats: HabiticaStats) {
  ...
  img.onload = () => {
    ...
    // Draw progress bars
    drawProgressBar(ctx, '❤️ Health', stats.hp, stats.maxHealth, '#F74E52', 20, 120)
    drawProgressBar(ctx, '⭐ Experience', stats.exp, stats.toNextLevel, '#FFB445', 20, 200)
    drawProgressBar(ctx, '💎 Mana', stats.mp, stats.maxMP, '#50B5E9', 20, 280)
  }
  img.src = "https://img.utdstc.com/icon/e99/6e9/e996e9fc7515afb60de11013c71209a2f12cf50d7ca67d88fb5fa280cf2f83c2:200"
}

function drawProgressBar(ctx: CanvasRenderingContext2D, label: string, value: number, max: number, color: string, x: number, y: number) {
  const width = 560
  const height = 20
  ...
  ctx.fillText(`${Math.floor(value)} / ${max}`, x + width - ctx.measureText(`${Math.floor(value)} / ${max}`).width, y - 10)
